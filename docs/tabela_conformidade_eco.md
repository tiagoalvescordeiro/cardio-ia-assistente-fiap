# Tabela de conformidade — CardioIA + ECO (auditoria FIAP Cap. 1)

Status: **Conforme** | **Parcialmente conforme** | **Não conforme**  
Evidência aponta para código no repositório, não para trechos soltos.
...
| 2 | Skill Watson exportável com intents, entidades e nós condicionais | **Conforme** | `watson/cardio_assistant_skill.json`: intents novos e aliases (`#sintoma_cardiaco`, `#red_flags`, `#estresse_ansiedade`, `#solicitar_respiracao`, `#ideacao_risco` + nomes anteriores); `@sinais_alerta` / `@sintomas_somaticos` e aliases; fork 192 vs 4-7-8 |
| 3 | UI fala com Flask; Calm UI / Dark Mode; disclaimer de não diagnóstico | **Conforme** | `frontend/script.js` → `POST /api/chat`; `index.html` + `styles.css` (`data-theme="dark"`, paleta fria); aviso legal permanente (não diagnostica / não é dispositivo médico) |
...
| 5 | Acolhimento + psicoeducação autonômica (luta-ou-fuga compartilhada) | **Conforme** | `REPLY_ECO_SCREEN` em `watson_service.py` e nós ECO do skill: luta-ou-fuga / sistema nervoso autônomo; só após rastreio negativo |
